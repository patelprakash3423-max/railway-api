...
export type SessionStatistics = Pick<AvailabilityDiagnostics,'availabilityBudgetLimit'|'availabilityRequestsUsed'|'availabilityCacheHits'|'budgetRemaining'|'classChecksByClass'|'availableResponses'|'racResponses'|'waitlistResponses'|'unavailableResponses'|'unsupportedClassResponses'|'providerErrors'|'providerErrorCategories'>;
...
export class AvailabilitySession {
  ...
  private readonly counts: Omit<SessionStatistics,'availabilityBudgetLimit'|'availabilityRequestsUsed'|'budgetRemaining'> = { availabilityCacheHits:0,classChecksByClass:{},availableResponses:0,racResponses:0,waitlistResponses:0,unavailableResponses:0,unsupportedClassResponses:0,providerErrors:0,providerErrorCategories:{} };
  ...
  statistics(): SessionStatistics { return { ...this.counts, classChecksByClass:{...this.counts.classChecksByClass},providerErrorCategories:{...this.counts.providerErrorCategories},availabilityBudgetLimit:this.limit,availabilityRequestsUsed:this.budget.callsUsed,budgetRemaining:this.remaining }; }
  async get(request: AvailabilityRequest): Promise<InventoryCheck> {
    ...
    const task=Promise.resolve().then(async()=>{
      ...
      try{result=normalizeInventory(r,await this.provider.getAvailability({...r}));}catch(error){
