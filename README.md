...
Availability SDK concurrency defaults to 2 (`RAILKIT_AVAILABILITY_MAX_CONCURRENT`). Waiting search owners rotate after each start; running work is not preempted. Identical train/from/to/date/class/quota requests share one SDK invocation across searches and provider instances. The shared cache defaults to 15 seconds (`RAILKIT_AVAILABILITY_CACHE_TTL_MS`) and 500 entries (`RAILKIT_AVAILABILITY_CACHE_MAX_ENTRIES`), evicts oldest insertions, and never serves expired entries. Only successfully normalized evidence with exactly one matching journey date and AVAILABLE, RAC, WAITLIST or NOT_AVAILABLE is cached. Generic errors, invalid/empty evidence, unsupported booking/class and local configuration failures are excluded from this inventory cache. AVAILABLE/RAC with canBook=false is also excluded. The raw SDK envelope is retained for raw-tool compatibility, validated through the same normalizer before caching, and cloned on reuse. Per-search cache semantics (including cached failures) remain unchanged.

Trustworthy Phase A `UNSUPPORTED_CLASS` evidence uses a separate policy bucket inside the same scheduler cache mechanism: `RAILKIT_UNSUPPORTED_CACHE_TTL_MS=900000` (15 minutes) and `RAILKIT_UNSUPPORTED_CACHE_MAX_ENTRIES=1000`. Both settings require positive safe integers; TTL is capped at 86400000 ms (24 hours), capacity at 10000 entries. Invalid configured values fail startup; absent/blank settings use defaults. Inventory TTL/capacity are independent and unchanged. Both buckets remove expired entries on access/insertion, evict oldest insertions at capacity, clone stored/replayed evidence, and do not extend TTL or refresh eviction order on hits.

The canonical exact-request key includes train number (trimmed, preserving leading zeroes), from/to station codes, journey/boarding date, class and quota. Codes are trimmed/uppercased and valid day-first or ISO dates canonicalize to DD-MM-YYYY. This key is shared by scheduler deduplication and the search-local cache; it does not relax API/provider validation (the public API still supports GN only). Unsupported evidence never excludes another route, date, train, class or quota and never populates `supportedClassesByTrain`. Unknown classes remain eligible. Generic HTTP 400, authentication errors, rate limits, server errors, timeouts, network/schema/unknown failures and all inventory statuses are excluded from the unsupported bucket. Configuration validation still runs before cache access.

An unsupported evidence hit consumes one distinct check in a new search's existing 12/30/40 budget, just like shared inventory reuse, but invokes no SDK and consumes no provider quota. Repetition within the same search uses its existing local cache without another budget charge. Concurrent identical requests still share the same pending invocation; one cancelled waiter cannot cancel peers, and abandoned/timed-out work cannot publish late evidence. These caches are process-local, reset at restart, and do not share evidence between instances.
...
V2 diagnostics remain opt-in through `EXPOSE_SEARCH_DIAGNOSTICS` or `ENABLE_API_DIAGNOSTICS`. The Phase B counters are internal/log-only, leaving the public journey response schema unchanged. Existing fields remain compatible; none of the check counters prove external HTTP requests or provider billing.
...
| `unsupportedEvidenceCacheHits` (internal diagnostics/completion and rejection logs only) | Fresh exact unsupported evidence replays from the scheduler; separate from inventory `sharedCacheHits`, search-local `cacheHits` and class-filter skips. |
| `providerUnsupportedResponses` (internal diagnostics/completion and rejection logs only) | Fresh unsupported results established by scheduler invocations, attributed once to their initiating search. Shared followers and cache replays record zero; rejected searches record zero. |
